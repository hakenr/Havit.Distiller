...
using Havit.Distiller.DataLayer.DataSources.Security;
using Havit.Distiller.DataLayer.Seeds.Core.Common;
using Havit.Distiller.Entity;
using Havit.Distiller.Services.Infrastructure;
using Havit.Distiller.Services.Infrastructure.TimeService;
...
namespace Havit.Distiller.WindsorInstallers
{
	public static class WindsorInstaller
	{
		...
		private static void InstallHavitServices(IWindsorContainer container)
		{
			...
			container.Register(Component.For<ICacheService>().Instance(new ObjectCacheService(new MemoryCache("Distiller"), false)));
		}
